...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_200.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    m1 = sum(x == "-1" for x in test_out)
    z = sum(x == "0" for x in test_out)
    o = sum(x == "1" for x in test_out)
    t = sum(x == "2" for x in test_out)

    print(f" - -1: {yellow}{m1:_}{reset}")
    print(f" -  0: {yellow}{z:_}{reset}")
    print(f" -  1: {yellow}{o:_}{reset}")
    print(f" -  2: {yellow}{t:_}{reset}")
