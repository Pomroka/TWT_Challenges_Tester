...
```sh
python test_challenge_138.py --help
```
...
```py
OTHER_LANG_COMMAND = "Cpp/c138_cpp.exe"  # relative to tester file path to compiled windows executable
OTHER_LANG_COMMAND = "/home/user/Dev/Challenge138/c138_c"  # absolute path to compiled Linux executable
OTHER_LANG_COMMAND = "c138_rust.exe"  # name of compiled file in same folder as tester
OTHER_LANG_COMMAND = "java -cp Java/ c138_java.Main"  # command to run solution in non compiled language
OTHER_LANG_COMMAND = ""  # leave empty if you want to test python solution
```

```sh
> python test_challenge_138.py -c "java -cp Java/ c138_java.Main"

$ python test_challenge_138.py -c Rust/c138_rust/target/release/c138_rust
```
...
- `StopIteration`: Your solution try to get more input than there is test cases
  ...
  - to avoid this use one of:
    - set `OTHER_LANG_COMMAND = "python to_submit_ch_138.py"`
    - run `python test_challenge_138.py -c "python to_submit_ch_138.py"`
...
```sh
# Linux
$ wget https://raw.githubusercontent.com/Pomroka/TWT_Challenges_Tester/master/Challenge_138/test_cases.json

# Windows 10
> curl -o test_cases.json https://raw.githubusercontent.com/Pomroka/TWT_Challenges_Tester/master/Challenge_138/test_cases.json
```
