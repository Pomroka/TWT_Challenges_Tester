...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_206.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_s = max(max(map(int, x[1].split())) for x in test_inp)
    avg_s = sum(sum(map(int, x[1].split())) // int(x[0]) for x in test_inp) // num_cases

    print(f" - Max length of an N: {yellow}{max_n:_}{reset}")
    print(f" - Average length of an N: {yellow}{avg_n:_}{reset}")
    print(f" - Max K: {yellow}{max_s:_}{reset}")
    print(f" - Average K: {yellow}{avg_s:_}{reset}")
