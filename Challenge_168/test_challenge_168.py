...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_168.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    max_s = max(len(x[0]) for x in test_inp)
    avg_s = sum(len(x[0]) for x in test_inp) // num_cases
    max_bp = max(int(x) for x in test_out)
    avg_bp = sum(int(x) for x in test_out) // num_cases

    print(f" - Max S len: {yellow}{max_s:_}{reset}")
    print(f" - Average S len: {yellow}{avg_s:_}{reset}")
    print(f" - Max bread len: {yellow}{max_bp:_}{reset}")
    print(f" - Average bread len: {yellow}{avg_bp:_}{reset}")
