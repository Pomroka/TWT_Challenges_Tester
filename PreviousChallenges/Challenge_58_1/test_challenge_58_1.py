...
from test_cases_ch_58_1 import test_inp, test_out
...
@mock.patch("builtins.input", side_effect=[str(1)] + test_inp)
def test_challenge_58_1(input: Callable) -> None:
    ...
    with Capturing() as output:
        ...
        import to_submit_ch_58_1  # change this to your file name with solution without .py extension

        end = perf_counter()

    from pprint import pprint
    
    out = [r for r in output]
    
    import hashlib
    
    hashed = hashlib.sha512("".join(out).encode("utf-8")).hexdigest()
    
    color = green if hashed == test_out else red
    ...
    if hashed == test_out:
        print(f"\n\n{color}Congratulations! Your solution solved my maze!{reset}")
    else:
        print(f"{color}Fail! You got lost somewhere in maze!{reset}")
