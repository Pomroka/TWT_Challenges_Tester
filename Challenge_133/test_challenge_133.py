...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_133.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    inc = sum(x == "inc" for x in test_out)
    dec = sum(x == "dec" for x in test_out)
    bad = sum(x == "bad" for x in test_out)

    print(f" - Max N: {yellow}{max_n:_}{reset}")
    print(f" - Average N: {yellow}{avg_n:_}{reset}")
    print(f" - Number of 'inc': {yellow}{inc:_}{reset}")
    print(f" - Number of 'dec': {yellow}{dec:_}{reset}")
    print(f" - Number of 'bad': {yellow}{bad:_}{reset}")
