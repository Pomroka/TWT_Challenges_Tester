...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_98.py"
    ...
    # TEST_CASE_FILE = "test_cases_b.json"  # 10000 cases
...
def print_extra_stats(
    ...
    print(f" - Max X or Y: {yellow}" f"{max(max(map(int, x[0].split())) for x in test_inp):_}{reset}")
    print(
        f" - Average X or Y: {yellow}"
        f"{sum(sum(map(int, x[0].split())) for x in test_inp) // (num_cases * 2):_}{reset}"
    )
