...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_183.py"
    ...
    # TEST_CASE_FILE = "test_cases.json"     # 100 cases max b length 952
    TEST_CASE_FILE = "test_cases_s.json"   # 500 cases max b length 100
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    max_a = max(len(x[0]) for x in test_inp)
    avg_a = sum(len(x[0]) for x in test_inp) // num_cases
    max_b = max(len(x[1]) for x in test_inp)
    avg_b = sum(len(x[1]) for x in test_inp) // num_cases

    print(f" - Max A lenght: {yellow}{max_a:_}{reset}")
    print(f" - Average A length: {yellow}{avg_a:_}{reset}")
    print(f" - Max B lenght: {yellow}{max_b:_}{reset}")
    print(f" - Average B length: {yellow}{avg_b:_}{reset}")
