...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_139.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    min_g = min(int(x) for x in test_out)
    max_g = max(int(x) for x in test_out)

    print(f" - Max N: {yellow}{max_n:_}{reset}")
    print(f" - Average N: {yellow}{avg_n:_}{reset}")
    print(f" - Avib min total grade: {yellow}{min_g:_}{reset}")
    print(f" - Avib max total grade: {yellow}{max_g:_}{reset}")
