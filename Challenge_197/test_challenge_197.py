...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_197.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_k = max(int(x[0].split()[0]) for x in test_inp)
    avg_k = sum(int(x[0].split()[0]) for x in test_inp) // num_cases
    max_r = max(int(x) for x in test_out)
    avg_r = sum(int(x) for x in test_out) // num_cases
    ...
    print(f" - Max erases: {yellow}{max_r:_}{reset}")
    print(f" - Average erases: {yellow}{avg_r:_}{reset}")
