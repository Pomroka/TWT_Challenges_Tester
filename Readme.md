...
```sh
python test_challenge_119.py --help
```
...
```py
OTHER_LANG_COMMAND = "Cpp/c119_cpp.exe"  # relative to tester file path to compiled windows executable
OTHER_LANG_COMMAND = "/home/user/Dev/Challenge119/c119_c"  # absolute path to compiled Linux executable
OTHER_LANG_COMMAND = "c119_rust.exe"  # name of compiled file in same folder as tester
OTHER_LANG_COMMAND = "java -cp Java/ c119_java.Main"  # command to run solution in non compiled language
OTHER_LANG_COMMAND = ""  # leave empty if you want to test python solution
```

```sh
> python test_challenge_119.py -c "java -cp Java/ c119_java.Main"

$ python test_challenge_119.py -c Rust/c119_rust/target/release/c119_rust
```
...
- `StopIteration`: Your solution try to get more input than there is test cases
  ...
  - to avoid this use one of:
    - set `OTHER_LANG_COMMAND = "python to_submit_ch_118.py"`
    - run `python test_challenge_118.py -c "python to_submit_ch_118.py"`
- If you call your functions inside `if __name__ == '__main__':` your functions won't be called by default cos your solution is imported
  - to avoid this use one of:
    - set `OTHER_LANG_COMMAND = "python to_submit_ch_118.py"`
    - run `python test_challenge_118.py -c "python to_submit_ch_118.py"`
    - set `OTHER_LANG_COMMAND = "python to_submit_ch_119.py"`
    - run `python test_challenge_119.py -c "python to_submit_ch_119.py"`
- If you call your functions inside `if __name__ == '__main__':` your functions won't be called by default cos your solution is imported
  - to avoid this use one of:
    - set `OTHER_LANG_COMMAND = "python to_submit_ch_119.py"`
    - run `python test_challenge_119.py -c "python to_submit_ch_119.py"`
...
```sh
# Linux
$ wget https://raw.githubusercontent.com/Pomroka/TWT_Challenges_Tester/master/Challenge_119/test_cases.json

# Windows 10
> curl -o test_cases.json https://raw.githubusercontent.com/Pomroka/TWT_Challenges_Tester/master/Challenge_119/test_cases.json
```
