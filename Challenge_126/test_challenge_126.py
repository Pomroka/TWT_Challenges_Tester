...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_126.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    print(f" - Max N/M: {yellow}" f"{max(max(map(int,x[0].split())) for x in test_inp):_}{reset}")
    print(
        f" - Average N: {yellow}"
        f"{sum(sum(map(int,x[0].split())) // 2 for x in test_inp) // num_cases:_}{reset}"
    )
    print(
        f" - Max result: {yellow}"
