...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_212.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    max_n = max(len(x[0].split()[0]) for x in test_inp)
    avg_n = sum(len(x[0].split()[0]) for x in test_inp) // num_cases
    max_b = max(int(x) for x in test_out)
    min_b = min(int(x) for x in test_out)
    avg_b = sum(int(x) for x in test_out) // num_cases

    print(f" - Max S len: {yellow}{max_n:_}{reset}")
    print(f" - Average S len: {yellow}{avg_n:_}{reset}")
    print(f" - Max beauty score: {yellow}{max_b:_}{reset}")
    print(f" - Min beauty score: {yellow}{min_b:_}{reset}")
    print(f" - Average beauty score: {yellow}{avg_b:_}{reset}")
