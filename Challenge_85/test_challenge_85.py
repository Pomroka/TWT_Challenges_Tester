...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_85.py"
...
def print_extra_stats(
    ...
    print(
        ...
        f"{sum(len(x[2].split()) for x in test_inp) // num_cases:_}{reset}"
    )
    print(
        f" - Max number of energy shortage days: {yellow}"
        f"{max(len(x[2].split()) for x in test_inp):_}{reset}"
    )
    print(
        f" - Average battery energy needed: {yellow}"
        f"{sum(int(x) for x in test_out) // num_cases:_}{reset}"
    )
    print(
        f" - Max battery energy needed: {yellow}"
