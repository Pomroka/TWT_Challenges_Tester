...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_121.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    print(f" - Max N: {yellow}" f"{max(int(x[0].split()[1]) for x in test_inp):_}{reset}")
    print(
        f" - Average N: {yellow}"
        f"{sum(int(x[0].split()[1]) for x in test_inp) // num_cases:_}{reset}"
    )
    print(f" - Player 1 wins: {yellow}" f"{sum(x != '0' for x in test_out) / num_cases * 100:.2f}%{reset}")
