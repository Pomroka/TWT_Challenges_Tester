...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_171.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_c = max(int(x) for x in test_out)
    avg_c = sum(int(x) for x in test_out) // num_cases

    print(f" - Max heroes: {yellow}{max_n:_}{reset}")
    print(f" - Average heroes: {yellow}{avg_n:_}{reset}")
    print(f" - Max B: {yellow}{max_m:_}{reset}")
    print(f" - Average B: {yellow}{avg_m:_}{reset}")
    print(f" - Max coins needed: {yellow}{max_c:_}{reset}")
    print(f" - Average coins needed: {yellow}{avg_c:_}{reset}")
