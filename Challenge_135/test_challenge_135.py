...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_135.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    max_q = max(int(x[1]) for x in test_inp)
    avg_q = sum(int(x[1]) for x in test_inp) // num_cases
    max_w = max(max(map(len, x[2:])) for x in test_inp)
    avg_w = sum(sum(map(len, x[2:])) // int(x[1]) for x in test_inp) // num_cases

    print(f" - Max Q: {yellow}{max_q:_}{reset}")
    print(f" - Average Q: {yellow}{avg_q:_}{reset}")
    print(f" - Max word lenght: {yellow}{max_w:_}{reset}")
    print(f" - Average word lenght: {yellow}{avg_w:_}{reset}")
