...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_130.py"
    ...
    TEST_CASE_FILE = "test_cases.json.gz"      # 1000 cases max n 49785
    # TEST_CASE_FILE = "test_cases_s.json.gz"  # 500 cases max n 9966
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_e = max(max(map(int, x[1].split())) for x in test_inp)
    avg_e = (
        sum(sum(map(int, x[1].split())) // len(x[1].split()) for x in test_inp)
        // num_cases
    )

    print(f" - Max len A: {yellow}{max_n:_}{reset}")
    print(f" - Average len A: {yellow}{avg_n:_}{reset}")
    print(f" - Max A element: {yellow}{max_e:_}{reset}")
    print(f" - Average A element: {yellow}" f"{avg_e:_}{reset}")
