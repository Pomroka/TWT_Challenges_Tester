...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_83.py"
    ...
    # Examples:
    # OTHER_LANG_COMMAND = "Cpp/c83_cpp.exe"
...
def print_extra_stats(
    ...
    print(
        f" - Average number of words: {yellow}"
        f"{sum(len(x[0].split()) for x in test_inp) // num_cases:_}{reset}"
    )
    print(
        f" - Max number of words: {yellow}"
        f"{max(len(x[0].split()) for x in test_inp)}{reset}"
    )
