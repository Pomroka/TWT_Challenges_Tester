...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_175.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_p = max(int(x[0].split()[1]) for x in test_inp)
    avg_p = sum(int(x[0].split()[1]) for x in test_inp) // num_cases
    avg_b = sum(int(x[0].split()[0]).bit_count() for x in test_inp) // num_cases

    print(f" - Max N: {yellow}{max_n:_}{reset}")
    print(f" - Average N: {yellow}{avg_n:_}{reset}")
    print(f" - Max P: {yellow}{max_p:_}{reset}")
    print(f" - Average P: {yellow}{avg_p:_}{reset}")
    print(f" - Average number of bits set in N: {yellow}{avg_b:_}{reset}")
