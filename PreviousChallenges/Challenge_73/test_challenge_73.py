...
SOLUTION_FILE_NAME = "to_submit_ch_73.py"
...
def print_extra_stats(test_inp: List[List[str]], num_cases: int) -> None:
    print(
        f" - Average length: {yellow}{sum(int(x[0]) for x in test_inp) // num_cases:_}"
        f"{reset}"
    )
    print(f" - Max length: {yellow}{max(int(x[0]) for x in test_inp):_}{reset}")
