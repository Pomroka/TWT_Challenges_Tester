...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_180.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_m = max(int(x[0].split()[1]) for x in test_inp)
    avg_m = sum(int(x[0].split()[1]) for x in test_inp) // num_cases
    max_diff = max(int(x[0].split()[1]) - int(x[0].split()[0]) for x in test_inp)
    min_diff = min(int(x[0].split()[1]) - int(x[0].split()[0]) for x in test_inp)
    avg_diff = sum(int(x[0].split()[1]) - int(x[0].split()[0]) for x in test_inp) // num_cases

    print(f" - Max N: {yellow}{max_n:_}{reset}")
    print(f" - Average N: {yellow}{avg_n:_}{reset}")
    print(f" - Max M: {yellow}{max_m:_}{reset}")
    print(f" - Average M: {yellow}{avg_m:_}{reset}")
    print(f" - Max diff beetwen N and M: {yellow}{max_diff:_}{reset}")
    print(f" - Min diff beetwen N and M: {yellow}{min_diff:_}{reset}")
    print(f" - Average dif beetween N and M: {yellow}{avg_diff:_}{reset}")
    ...
    for i, (inp, exp) in enumerate(
        ...
        if out != exp:
            if i - passed <= Config.NUM_FAILED or Config.NUM_FAILED == -1:
                ...
                truncated = False
                for line in inp:
                    truncated = truncated or len(line) > max_line_len
                    print(line[:max_line_len] + " ..." if len(line) > max_line_len else line)
                if truncated:
                    print(
                        f"{reset}{emojis.warning}Printed input is truncated to {yellow}"
                        f"{max_line_len}{reset} characters per line{emojis.warning}"
                    )
                    print(
                        f"\rTo change how many characters to print change "
                        f"{cyan}TRUNCATE_FAILED_CASES{reset} in configuration section.\n"
                    )
                print(
...
def parse_args() -> None:
    ...
    parser.add_argument(
        ...
        metavar="<int>",
        type=int,
        help="Number of times to run tests to get more accurate times.",
    )
    parser.add_argument(
        "--number-speed-cases",
        metavar="<int>",
