...
from test_cases_ch_55_2 import test_inp, test_out
...
def test_challenge_55_2(input: Callable) -> None:
    ...
    with Capturing() as output:
        ...
        import to_submit_ch_55_2  # change this to your file name with solution without .py extension
    ...
    for i, (out, exp) in enumerate(zip_longest(output, test_out), 1):
        if out != exp:
            print(f"\nTest nr:{i}\n      Input: {cyan}{test_inp[(i - 1) * 2]}")
            print(f"\t     {test_inp[(i - 1) * 2 + 1]}")
