...
```sh
python test_challenge_126.py --help
```
...
```py
OTHER_LANG_COMMAND = "Cpp/c126_cpp.exe"  # relative to tester file path to compiled windows executable
OTHER_LANG_COMMAND = "/home/user/Dev/Challenge126/c126_c"  # absolute path to compiled Linux executable
OTHER_LANG_COMMAND = "c126_rust.exe"  # name of compiled file in same folder as tester
OTHER_LANG_COMMAND = "java -cp Java/ c126_java.Main"  # command to run solution in non compiled language
OTHER_LANG_COMMAND = ""  # leave empty if you want to test python solution
```

```sh
> python test_challenge_126.py -c "java -cp Java/ c126_java.Main"

$ python test_challenge_126.py -c Rust/c126_rust/target/release/c126_rust
```
...
- `StopIteration`: Your solution try to get more input than there is test cases
  ...
  - to avoid this use one of:
    - set `OTHER_LANG_COMMAND = "python to_submit_ch_126.py"`
    - run `python test_challenge_126.py -c "python to_submit_ch_126.py"`
...
```sh
# Linux
$ wget https://raw.githubusercontent.com/Pomroka/TWT_Challenges_Tester/master/Challenge_126/test_cases.json

# Windows 10
> curl -o test_cases.json https://raw.githubusercontent.com/Pomroka/TWT_Challenges_Tester/master/Challenge_126/test_cases.json
```
