...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_187.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_m = max(len(x[2]) for x in test_inp)
    avg_m = sum(len(x[2]) for x in test_inp) // num_cases

    print(f" - Max N: {yellow}{max_n:_}{reset}")
    print(f" - Average N: {yellow}{avg_n:_}{reset}")
    print(f" - Max moves: {yellow}{max_m:_}{reset}")
    print(f" - Average moves: {yellow}{avg_m:_}{reset}")
    pass
    pass
