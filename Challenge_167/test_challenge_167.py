...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_167.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    max_a = max(int(x[0].split()[0]) for x in test_inp)
    avg_a = sum(int(x[0].split()[0]) for x in test_inp) // num_cases
    max_b = max(int(x[0].split()[1]) for x in test_inp)
    avg_b = sum(int(x[0].split()[1]) for x in test_inp) // num_cases
    max_bp = max(int(x) for x in test_out)
    avg_bp = sum(int(x) for x in test_out) // num_cases

    print(f" - Max A: {yellow}{max_a:_}{reset}")
    print(f" - Average A: {yellow}{avg_a:_}{reset}")
    print(f" - Max B: {yellow}{max_b:_}{reset}")
    print(f" - Average B: {yellow}{avg_b:_}{reset}")
    print(f" - Max Baby Palindrome: {yellow}{max_bp:_}{reset}")
    print(f" - Average Baby Palindrome: {yellow}{avg_bp:_}{reset}")
