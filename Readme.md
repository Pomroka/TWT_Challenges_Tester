...
```sh
python test_challenge_123.py --help
```
...
```py
OTHER_LANG_COMMAND = "Cpp/c123_cpp.exe"  # relative to tester file path to compiled windows executable
OTHER_LANG_COMMAND = "/home/user/Dev/Challenge123/c123_c"  # absolute path to compiled Linux executable
OTHER_LANG_COMMAND = "c123_rust.exe"  # name of compiled file in same folder as tester
OTHER_LANG_COMMAND = "java -cp Java/ c123_java.Main"  # command to run solution in non compiled language
OTHER_LANG_COMMAND = ""  # leave empty if you want to test python solution
```

```sh
> python test_challenge_123.py -c "java -cp Java/ c123_java.Main"

$ python test_challenge_123.py -c Rust/c123_rust/target/release/c123_rust
```
...
- `StopIteration`: Your solution try to get more input than there is test cases
  ...
  - to avoid this use one of:
    - set `OTHER_LANG_COMMAND = "python to_submit_ch_123.py"`
    - run `python test_challenge_123.py -c "python to_submit_ch_123.py"`
...
```sh
# Linux
$ wget https://raw.githubusercontent.com/Pomroka/TWT_Challenges_Tester/master/Challenge_123/test_cases.json

# Windows 10
> curl -o test_cases.json https://raw.githubusercontent.com/Pomroka/TWT_Challenges_Tester/master/Challenge_123/test_cases.json
```
