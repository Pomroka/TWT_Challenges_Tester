...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_203.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_a = max(max(int(n) for n in x[1].split()) for x in test_inp)
    avg_a = sum(sum(int(n) for n in x[1].split()) // len(x[1].split()) for x in test_inp) // num_cases

    print(f" - Max N: {yellow}{max_n:_}{reset}")
    print(f" - Average N: {yellow}{avg_n:_}{reset}")
    print(f" - Max element of A: {yellow}{max_a:_}{reset}")
    print(f" - Average element of A: {yellow}{avg_a:_}{reset}")
