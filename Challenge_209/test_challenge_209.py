...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_209.py"
    ...
    TEST_CASE_FILE = "test_cases_s.json.gz"  # 100 cases max k -  27_764
    TEST_CASE_FILE = "test_cases.json.gz"    # 500 cases max k - 129_744
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_k = max(int(x[0].split()[2]) for x in test_inp)
    avg_k = sum(int(x[0].split()[2]) for x in test_inp) // num_cases
    zero = sum(x == "0" for x in test_out)
    ...
    print(f" - Max K: {yellow}{max_k:_}{reset}")
    print(f" - Average K: {yellow}{avg_k:_}{reset}")
    print(f" - Zero results: {yellow}{zero:_}{reset}")
