...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_156.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    yes = sum(x == "YES" for x in test_out)
    no = sum(x == "NO" for x in test_out)

    print(f" - Max length of pond: {yellow}{max_n:_}{reset}")
    print(f" - Average length of pond: {yellow}{avg_n:_}{reset}")
    print(f" - Max jump range: {yellow}{max_m:_}{reset}")
    print(f" - Average jump range: {yellow}{avg_m:_}{reset}")
    print(f" - Tom reached other side: {yellow}{yes:_}{reset} times")
    print(f" - Tom didn't reach other side: {yellow}{no:_}{reset} times")
