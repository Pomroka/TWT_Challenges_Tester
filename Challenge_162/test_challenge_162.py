...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_162.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_p = max(int(x) for x in test_out)
    avg_p = sum(int(x) for x in test_out) // num_cases

    print(f" - Max Paries: {yellow}{max_n:_}{reset}")
    print(f" - Average Paries: {yellow}{avg_n:_}{reset}")
    print(f" - Max Parties attended: {yellow}{max_p:_}{reset}")
    print(f" - Average Parties attended: {yellow}{avg_p:_}{reset}")
