...
from test_cases_ch_57_2 import test_inp, test_out
...
def test_challenge_57_2(input: Callable) -> None:
    ...
    with Capturing() as output:
        ...
        import to_submit_ch_57_2  # change this to your file name with solution without .py extension
    ...
    for i in range(1, 21):
        ...
        out, exp = set(output[i - 1].split()), [set(x.split()) for x in test_out[i - 1]]
        ii += c + 1
        if out not in exp:
            print(f"Test nr:{i}\n      Input: {cyan}")
            pprint(inp)
            print(f"{reset}Your output: {red}{out}{reset}")
            print(f"Valid expected sets of jobs: {green}{exp}{reset}")
        else:
            passed += 1
