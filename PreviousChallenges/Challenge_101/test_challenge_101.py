...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_101.py"
...
def print_extra_stats(
    ...
    print(f" - Max Q: {yellow}" f"{max(int(x[0].split()[1]) for x in test_inp):_}{reset}")
    print(f" - Max N * Q: {yellow}" f"{max(int(x[0].split()[0]) * int(x[0].split()[1]) for x in test_inp):_}{reset}")
    print(
        f" - Average N * Q: {yellow}"
