...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_185.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_k = max(int(x[0].split()[1]) for x in test_inp)
    avg_k = sum(int(x[0].split()[1]) for x in test_inp) // num_cases
    max_p = max(int(x.split("/")[0]) / int(x.split("/")[1]) for x in test_out)
    avg_p = sum(int(x.split("/")[0]) / int(x.split("/")[1]) for x in test_out) / num_cases
    ...
    print(f" - Max probability: {yellow}{max_p:_}{reset}")
    print(f" - Average probability: {yellow}{avg_p:.3}{reset}")
