...
from test_cases_ch_56_1 import test_inp, test_out, test_num
...
@mock.patch("builtins.input", side_effect=[str(test_num)] + test_inp)
def test_challenge_56_1(input: Callable) -> None:
    ...
    with Capturing() as output:
        ...
        import to_submit_ch_56_1  # change this to your file name with solution without .py extension
    ...
    for i in range(1, test_num + 1):
        ...
        oi += c
        if out != exp:
            print(f"{reset}Test nr: {i}\n      Input: {cyan}")
            pprint(inp)
            print(f"{reset}Your output: {red}")
            pprint(out)
            print(f"{reset}   Expected: {green}")
            pprint(exp)
        else:
            passed += 1

    print(f"\n{reset}Passed: {green if passed == i else red}{passed}/{i}{reset} tests")
