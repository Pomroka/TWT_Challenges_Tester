...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_208.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_a = max(max(int(y) for y in x.split()) for x in test_inp[1])

    print(f" - Max N: {yellow}{max_n:_}{reset}")
    print(f" - Average N: {yellow}{avg_n:_}{reset}")
    print(f" - Max queries: {yellow}{max_q:_}{reset}")
    print(f" - Average queries: {yellow}{avg_q:_}{reset}")
