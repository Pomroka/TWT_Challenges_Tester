...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_132.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    max_m = max(int(x[0].split()[0]) for x in test_inp)
    max_q = max(int(x[0].split()[1]) for x in test_inp)
    avg_m = sum(int(x[0].split()[0]) for x in test_inp) // num_cases
    avg_q = sum(len(x[0].split()[1]) for x in test_inp) // num_cases
