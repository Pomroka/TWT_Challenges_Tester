...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_172.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_m = max(max(int(y.split()[0]) for y in x[1:]) for x in test_inp)
    avg_m = sum(sum(int(y.split()[0]) // int(x[0]) for y in x[1:]) for x in test_inp) // num_cases
    max_t = max(int(x) for x in test_out)
    avg_t = sum(int(x) for x in test_out) // num_cases
    no_s = sum(x == "-1" for x in test_out)

    print(f" - Max videos: {yellow}{max_n:_}{reset}")
    print(f" - Average videos: {yellow}{avg_n:_}{reset}")
    print(f" - Max video lenght: {yellow}{max_m:_}{reset}")
    print(f" - Average video lenght: {yellow}{avg_m:_}{reset}")
    print(f" - Max time needed: {yellow}{max_t:_}{reset}")
    print(f" - Average time needed: {yellow}{avg_t:_}{reset}")
    print(f" - Avib wont learn two skills: {yellow}{no_s:_}{reset}")
