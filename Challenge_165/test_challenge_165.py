...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_165.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    y = sum(x == "YES" for x in test_out)
    n = sum(x == "NO" for x in test_out)

    print(f" - Number of YES results: {yellow}{y:_}{reset}")
    print(f" - Number of NO results: {yellow}{n:_}{reset}")
