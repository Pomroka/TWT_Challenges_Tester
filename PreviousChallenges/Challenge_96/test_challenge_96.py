...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_96.py"
    ...
    TEST_CASE_FILE = "test_cases.json"         # 500 cases 1000 max 𝜋かጩ
    # TEST_CASE_FILE = "test_cases_b.json.gz"  # 500 cases 3000 max 𝜋かጩ
...
def print_extra_stats(
    ...
    print(
        f" - Max number of 𝜋かጩ: {yellow}"
        f"{max(int(x[0].split()[1]) for x in test_inp):_}{reset}"
    )
    print(
        f" - Average number of 𝜋かጩ: {yellow}"
        f"{sum(int(x[0].split()[1]) for x in test_inp) // num_cases:_}{reset}"
    )
