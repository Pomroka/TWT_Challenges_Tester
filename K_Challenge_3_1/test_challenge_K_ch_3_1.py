...
SOLUTION_FILE_NAME = "solution_k_3_1.py"
...
def print_extra_stats(test_inp: List[List[str]], num_cases: int) -> None:
    ...
    print(f" - Max length of blocks: {yellow}{max(len(x) for x in test_inp):_}{reset}")
    print(
        f" - Average length of facilities: {yellow}"
        f"{sum(len(x[1].split()) for x in test_inp) // num_cases:_}{reset}"
    )
    print(
        f" - Max length of facilities: {yellow}"
...
def speed_test_solution_aio(
    ...
    if PRINT_EXTRA_STATS:
        print_extra_stats(test_inp[:speed_num_cases], speed_num_cases)
    print()
