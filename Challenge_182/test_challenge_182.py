...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_182.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_a = max(max(map(int, x[1].split())) for x in test_inp)
    min_a = min(min(map(int, x[1].split())) for x in test_inp)
    yan = sum(x == "Yan" for x in test_out)

    print(f" - Max N: {yellow}{max_n:_}{reset}")
    print(f" - Average N: {yellow}{avg_n:_}{reset}")
    print(f" - Max sequence element: {yellow}{max_a:_}{reset}")
    print(f" - Min sequence element: {yellow}{min_a:_}{reset}")
    print(f" - Yan wins: {yellow}{yan:_}{reset}")
    print(f" - Tim wins: {yellow}{num_cases - yan:_}{reset}")
