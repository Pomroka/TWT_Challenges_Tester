...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_201.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    mi = min(int(x[0]) for x in test_inp)
    ma = max(int(x[0]) for x in test_inp)

    print(f"Min: {yellow}{mi:_}{reset}")
    print(f"Max: {yellow}{ma:_}{reset}")
