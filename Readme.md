...
```sh
python test_challenge_115.py --help
```
...
```py
OTHER_LANG_COMMAND = "Cpp/c115_cpp.exe"  # relative to tester file path to compiled windows executable
OTHER_LANG_COMMAND = "/home/user/Dev/Challenge115/c115_c"  # absolute path to compiled Linux executable
OTHER_LANG_COMMAND = "c115_rust.exe"  # name of compiled file in same folder as tester
OTHER_LANG_COMMAND = "java -cp Java/ c115_java.Main"  # command to run solution in non compiled language
OTHER_LANG_COMMAND = ""  # leave empty if you want to test python solution
```

```sh
> python test_challenge_115.py -c "java -cp Java/ c115_java.Main"

$ python test_challenge_115.py -c Rust/c115_rust/target/release/c115_rust
```
...
- `StopIteration`: Your solution try to get more input than there is test cases
  ...
  - to avoid this use one of:
    - set `OTHER_LANG_COMMAND = "python to_submit_ch_115.py"`
    - run `python test_challenge_115.py -c "python to_submit_ch_115.py"`
...
```sh
# Linux
$ wget https://raw.githubusercontent.com/Pomroka/TWT_Challenges_Tester/master/Challenge_115/test_cases.json

# Windows 10
> curl -o test_cases.json https://raw.githubusercontent.com/Pomroka/TWT_Challenges_Tester/master/Challenge_115/test_cases.json
```
