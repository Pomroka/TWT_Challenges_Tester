...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_169.py"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    max_s = max(int(x[0]) for x in test_inp)
    avg_s = sum(int(x[0]) for x in test_inp) // num_cases

    print(f" - Max N len: {yellow}{max_s:_}{reset}")
    print(f" - Average N len: {yellow}{avg_s:_}{reset}")
